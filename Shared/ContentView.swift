...
//  Shared
//
//  Created by Sai Lakshmi on 9/16/22.
//

import SwiftUI

struct Hexagonal: Shape {
    func path(in rect: CGRect) -> Path { // here rect is the entire frame
        return Path { path in
            ...
            path.move(to: p6) // it is mandatory otherwise the picture wont show
            //path.move(to: p6) also works
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .center, spacing: 80) {
            ...
            Button {
                ...
                RoundedRectangle(cornerRadius: 10)
                    ...
                    .shadow(color: .gray, radius: 5, y: 4)
                    .overlay(Text("Spin")
                                .fontWeight(.black)
                                .font(.title3)
        ...
        .alert(item: $showingAlert) { alert -> Alert in
            switch alert {
            case .success:
                return Alert(title: Text("Yay! You won!"), message: Text("Born With Charm"), dismissButton: .cancel())
            case .failure:
                return Alert(title: Text("Oops! You Lose."), message: Text("Next time!"), dismissButton: .cancel())
            }
        }
